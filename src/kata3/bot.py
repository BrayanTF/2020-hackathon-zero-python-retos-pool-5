...
def mayus(update, context):
        #
        texto_usuario = str(context.args[0]).upper()
        update.message.reply_text(texto_usuario)
...
def alreves(update, context):
    ...
    for x in range(0, len(texto)):
        texto_alrevez = texto_alrevez + texto[(len(texto) - 1) - x]
    update.message.reply_text(texto_alrevez)
    #
...
def main():
    ...
    updater = Updater("1140849073:AAHValGGrVPOQyBJv3smTc8gzI0ybCafTFM", use_context=True)
